...
bool readDecaytree(const std::string &fileName, TTree *tree) {
  ...
  if (!tree) {
    std::cerr << "Error: Could not find TTree in file " << fileName
              << std::endl;
    return false;
  }
...
void plotCutFlowHistogram(TTree *tree) {
  ...
  TH1F *cutFlowHistogram =
      new TH1F("cutFlowHistogram", "Cut Flow", AMOUNT_OF_CRITERIA, 0.5, 20.5);
  ...
  Double_t muon1_PT, muon1_ProbNNmu, muon1_ProbNNpi, muon1_ProbNNe,
      muon1_ProbNNp, muon2_PT, muon2_ProbNNmu, muon2_ProbNNpi, muon2_ProbNNe,
      muon2_ProbNNp, Z_MMERR;
